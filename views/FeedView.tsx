          ...
          <React.Fragment key={post.id}>
            ...
            <div className="glass-effect rounded-3xl md:rounded-[2.8rem] overflow-hidden border border-white/5 shadow-xl transition-all">
              <div className="p-5 md:p-10">
                <div className="flex items-center justify-between mb-5 md:mb-8"><Link to={`/profile/${post.authorId}`} className="flex items-center gap-3 md:gap-4"><img src={post.authorPhoto} className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl object-cover ring-2 md:ring-4 ring-slate-950" alt="a" /><div><div className="flex items-center gap-2"><span className="text-[11px] md:text-sm font-black text-slate-100 uppercase tracking-tight">{post.authorName}</span><UserBadge role={post.authorRole} /></div><span className="text-[7px] md:text-[8px] text-slate-600 uppercase tracking-widest font-black mt-0.5 block">{new Date(post.createdAt).toLocaleDateString()}</span></div></Link></div>
                
                {/* Specific Ad for Articles (Boro artical a banner) */}
                ...
                {post.imageUrl && <div className={`rounded-2xl md:rounded-[2rem] overflow-hidden border border-white/5 bg-slate-950 mt-5 md:mt-8 relative ${!isPremium ? 'grayscale blur-md select-none' : ''}`}><img src={post.imageUrl} className="w-full h-auto max-h-[400px] md:max-h-[600px] object-cover" alt="c" />{!isPremium && <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => navigate('/pro')}><span className="bg-amber-500 text-slate-950 text-[9px] font-black uppercase px-4 py-2 rounded-xl shadow-2xl cursor-pointer">Premium to Decrypt</span></div>}</div>}
                <div className="flex items-center gap-5 md:gap-8 mt-6 md:mt-10 pt-6 md:pt-8 border-t border-white/5"><button onClick={() => handleLike(post)} className={`flex items-center gap-2 text-[9px] md:text-[10px] font-black tracking-widest transition-all ${post.likes.includes(user.uid) ? 'text-rose-500' : 'text-slate-500'}`}><div className={`p-2 rounded-xl ${post.likes.includes(user.uid) ? 'bg-rose-500/10' : 'bg-slate-900'}`}><svg className="w-4 h-4 md:w-5 md:h-5" fill={post.likes.includes(user.uid) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg></div>{post.likes.length}</button><button onClick={() => setOpenComments(prev => ({ ...prev, [post.id]: !prev[post.id] }))} className={`flex items-center gap-2 text-[9px] md:text-[10px] font-black tracking-widest transition-all ${openComments[post.id] ? 'text-rose-400' : 'text-slate-500'}`}><div className={`p-2 rounded-xl ${openComments[post.id] ? 'bg-rose-500/10' : 'bg-slate-900'}`}><svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg></div>{post.commentsCount}</button></div>
