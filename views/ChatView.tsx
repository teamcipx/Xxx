...
const ChatView: React.FC<{ activeUser: User }> = ({ activeUser }) => {
  ...
  // Sync mobile view state with URL
  useEffect(() => {
  ...
  return (
    <div className="h-[calc(100vh-180px)] md:h-[75vh] flex glass-effect rounded-3xl md:rounded-[2.5rem] shadow-2xl border border-white/10 overflow-hidden animate-fadeIn">
      {/* Messenger List Sidebar / Mobile List View */}
      <div className={`${showListOnMobile ? 'flex' : 'hidden'} lg:flex flex-col w-full lg:w-80 border-r border-white/5 bg-slate-950/20`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
           ...
           {recentChats.map(rc => (
             <button 
               key={rc.chatId} 
               onClick={() => { navigate(`/chat/${rc.chatId}`); setShowListOnMobile(false); }}
               className={`w-full p-4 md:p-5 flex gap-4 items-center border-b border-white/5 transition-all text-left ${chatId === rc.chatId ? 'bg-indigo-600/10 border-r-4 border-r-indigo-500' : 'hover:bg-white/5'}`}
             >
                <img src={rc.partnerPhoto} className="w-10 h-10 md:w-11 md:h-11 rounded-2xl object-cover ring-2 ring-slate-950" alt="p" />
                <div className="flex-1 min-w-0">
                   <p className="text-[10px] md:text-[11px] font-black text-white uppercase truncate">{rc.partnerName}</p>
                   <p className="text-[9px] text-slate-500 truncate italic">{rc.lastMessage}</p>
                </div>
             </button>
           ))}
        </div>
      ...
      <div className={`${!showListOnMobile ? 'flex' : 'hidden'} lg:flex flex-1 flex-col bg-slate-950/40 relative`}>
        <div className="p-4 md:p-5 border-b border-white/5 flex items-center justify-between bg-slate-900/40 backdrop-blur-md">
          <div className="flex items-center gap-3 md:gap-4">
             <button 
               ...
               className="lg:hidden p-2 bg-slate-800 rounded-xl text-slate-400 hover:text-white transition-all active:scale-90"
             >
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
             </button>
             <div className={`w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center ${isPrivate ? 'bg-indigo-600' : 'bg-rose-600'} text-white shadow-lg`}>
             ...
             <div>
                <h2 className="font-black text-white tracking-tight uppercase text-[10px] md:text-xs">{isPrivate ? 'Private Line' : 'Global Lobby'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-4 md:space-y-6 custom-scrollbar" ref={scrollRef}>
          {loading ? (
            <div className="h-full flex flex-col items-center justify-center opacity-40"><div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin mb-3"></div><p className="text-[8px] font-black uppercase tracking-widest">establishing link</p></div>
          ) : messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-20"><p className="text-[10px] font-black uppercase tracking-widest">No signals in buffer</p></div>
          ) : (
            messages.map((m, i) => {
                   ...
                   <div className={`max-w-[90%] md:max-w-[70%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      {!isMe && (i === 0 || messages[i-1].senderId !== m.senderId) && (
                        <div className="flex items-center gap-2 mb-1.5 ml-1.5">
                           <Link to={`/profile/${m.senderId}`} className="text-[9px] font-black text-rose-500 uppercase hover:underline">{m.senderName}</Link>
                           <UserBadge role={m.senderRole} verified={m.senderVerified} />
                        </div>
                      )}
                      <div className={`px-4 py-3 rounded-2xl shadow-xl leading-relaxed text-[13px] md:text-sm ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-white/5'}`}>
        ...
        <div className="p-4 md:p-6 bg-slate-900/60 border-t border-white/5 flex gap-3 md:gap-4 backdrop-blur-md">
           <input 
             ...
             className="flex-1 bg-slate-950/80 border border-white/10 rounded-2xl px-4 md:px-6 py-3 md:py-4 text-[13px] md:text-xs font-bold text-white outline-none focus:border-rose-500/50"
           />
